...
//         // Assuming the admin name, email, and contact are not stored in the database
//       } else {
...
//       //   return;
//       // }
//       // if (!studentData.admissonyear || isNaN(studentData.admissonyear) || studentData.admissonyear.length !== 4) {
//       //   alert('Please enter a valid admission year.');
//       //   return;
//       // }
...
//     const studentIds = Array.from(document.getElementById('student-select').selectedOptions).map(option => option.value);
...
document.addEventListener('DOMContentLoaded', function() {
 ...
 // Fetch faculty for class assignment
  ...
  //   const batchYear = document.getElementById('search-year').value;

  //   if (!facultyIds.length || !courseId || !batchYear) {
  //     alert('Please select at least one faculty, one course, and enter a batch year.');
  //     return;
  //   }

  //   console.log('Sending request with data:', { facultyIds, courseId, batchYear }); // Add this line to log the request data
  ...
  //       'Content-Type': 'application/json',
  //     },
  //     body: JSON.stringify({ facultyIds, courseId, batchYear }),
  //   })
  //   .then(response => response.json())
  //   .then(data => {
  //     alert(data.message);

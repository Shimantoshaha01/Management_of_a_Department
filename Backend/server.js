...
// });
// app.get('/', (req, res) => {
//     res.sendFile(path.join(__dirname, 'Frontend', 'studentpage.html'));
// });
...
//             res.status(500).send('Server error');
//         } else {
//             res.json(result[0]); // Assuming a single faculty record
//         }
//     });
// });


// // Serve the studentpage.html file at the root path
// app.get('/', (req, res) => {
//     res.sendFile(path.join(__dirname, 'Frontend', 'Facultypage.html'));
// });
...
// Route to assign classes to faculty
// app.post("/api/assign-classes", (req, res) => {
//     const { facultyIds, courseId, batchYear } = req.body;

//     // Ensure that the facultyIds, courseId, and batchYear are provided
//     if (!facultyIds || facultyIds.length === 0 || !courseId || !batchYear) {
//         return res.status(400).json({ error: "Invalid input: Faculty IDs, course ID, and batch year are required." });
//     }
...
//         const query = "INSERT INTO Takes (Fac_id, Cou_code, Batch) VALUES (?, ?, ?)";

//         // Insert each selected faculty with the selected course and batch year inside the transaction
//         const promises = facultyIds.map((facultyId) => {
//             return new Promise((resolve, reject) => {
//                 db.query(query, [facultyId, courseId, batchYear], (err, result) => {
//                     if (err) {
...
//                   res.status(500).json({ error: "Error assigning classes to faculty." });
//               });
//           }
//           // Send success response
//           res.json({
//               message: "Classes assigned successfully to all selected faculty",
//           });
//       });
//   })
//   .catch((err) => {
